...
const Hero = () => {
  ...
  return (
    <section id="home" style={{
      ...
      height: 'calc(100vh - 116px)', marginTop: '116px',
      overflow: 'hidden', background: '#111',
    }}>
      {SLIDES.map((slide, i) => (
        <img key={slide.img} src={slide.img} alt=""
          style={{
            position: 'absolute', inset: 0, width: '100%', height: '100%',
            objectFit: 'fill',
            opacity: i === current ? 1 : 0,
            transition: 'opacity 0.5s ease', willChange: 'opacity',
          }}
        />
      ))}
...
const arrowStyle = (side) => ({
  position: 'absolute', [side]: '24px', top: '50%',
