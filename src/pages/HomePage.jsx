...
const Eyebrow = ({ children, light = false }) => (
  <div style={{
    ...
    marginBottom: '16px', fontFamily: P,
  }}>
    ...
    <div style={{ display: 'flex', width: 'max-content', animation: 'tickerMove 55s linear infinite' }}>
      {[...TICKER_ITEMS, ...TICKER_ITEMS].map((txt, i) => (
        <span key={i} style={{
          ...
          color: '#fff', padding: '0 40px', fontFamily: P,
        }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)' }}>
        {STATS.map((s, i) => (
          <Reveal key={i} delay={i * 0.08}>
            <div style={{ padding: '32px 24px', borderRight: i < STATS.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
              <div style={{ fontFamily: P, fontSize: '44px', fontWeight: 800, color: '#fff', lineHeight: 1, letterSpacing: '-0.02em', marginBottom: '8px' }}>
                {s.body}<span style={{ color: RED }}>{s.accent}</span>
              </div>
              <div style={{ fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.75)', marginBottom: '4px', fontFamily: P }}>{s.name}</div>
              <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', fontFamily: P }}>{s.hint}</div>
            </div>
          </Reveal>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginTop: '32px', paddingTop: '24px', borderTop: '1px solid rgba(255,255,255,0.06)', fontSize: '11px', color: 'rgba(255,255,255,0.35)', fontFamily: P }}>
...
const CEOSection = () => (
  <div style={{ background: 'linear-gradient(135deg, #f8f9fc 0%, #eef1f7 100%)', padding: '100px 0', position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 56px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '72px', alignItems: 'center' }}>
        <Reveal direction="left" delay={0.05}>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, background: NAVY, overflow: 'hidden' }}>
              <div style={{ position: 'relative', height: '340px', overflow: 'hidden', background: `linear-gradient(160deg,#1a2f5a,${NAVY})` }}>
              ...
              <div style={{ padding: '20px 28px 24px', borderTop: `3px solid ${RED}` }}>
                <div style={{ fontFamily: P, fontWeight: 700, fontSize: '16px', color: '#fff', marginBottom: '4px' }}>Muhammad Ahsan Naeem</div>
        ...
        <Reveal direction="right" delay={0.12}>
          ...
          <h3 style={{ fontFamily: P, fontSize: 'clamp(17px,1.9vw,24px)', fontWeight: 700, color: NAVY, lineHeight: 1.45, marginBottom: '24px', marginTop: '-12px' }}>
            We started with a simple belief —{' '}
            <em style={{ fontStyle: 'normal', color: RED }}>every fleet deserves to be smarter.</em>
          </h3>
          <p style={{ fontSize: '14px', color: '#4a5568', lineHeight: 1.85, margin: '0 0 16px', fontFamily: P }}>
            When I founded Timeline Telematics in 2012, the fleet management industry in Pakistan was still operating on paper logs and phone calls. Drivers were invisible once they left the depot. Fuel was leaking out of every operation.
          </p>
          <p style={{ fontSize: '14px', color: '#4a5568', lineHeight: 1.85, margin: 0, fontFamily: P }}>
          ...
          <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px solid #dde3ee', display: 'flex', alignItems: 'center', gap: '16px' }}>
...
const TIMELINE = [
  ...
  { year: '2018', label: 'Gulf Expansion', title: 'UAE & Gulf Launch',            desc: 'Dubai office opened. Arabic language support and UAE RTA integration launched. Hardware certified for Gulf heat extremes. First enterprise contracts with major Gulf logistics operators.' },
  ...
  { year: '2023', label: 'Scale',          title: '25,000+ Fleets Milestone',     desc: '25,000+ active fleet operators globally. 50M+ daily data points. Forbes Middle East recognition. Three continental offices fully operational with local language support.' },
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 56px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            {['←', '→'].map((arrow, i) => (
              <button key={i} onClick={() => goTo(cur + (i === 0 ? -1 : 1))}
                style={{ width: '40px', height: '40px', border: '1px solid rgba(255,255,255,0.15)', background: 'transparent', color: '#fff', cursor: 'pointer', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.25s' }}
        ...
        <div key={cur} style={{ display: 'grid', gridTemplateColumns: '200px 1fr', animation: 'fadeIn 0.4s ease' }}>
          <div style={{ background: RED, padding: '32px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', minHeight: '160px' }}>
            <div style={{ fontFamily: P, fontSize: '54px', fontWeight: 800, color: '#fff', lineHeight: 1, letterSpacing: '-0.03em' }}>{TIMELINE[cur].year}</div>
            <div style={{ fontSize: '9px', fontWeight: 700, letterSpacing: '3px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.6)', marginTop: '8px', fontFamily: P }}>{TIMELINE[cur].label}</div>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: 'none', padding: '36px 44px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <p style={{ fontSize: '13.5px', color: 'rgba(255,255,255,0.55)', lineHeight: 1.8, margin: 0, fontFamily: P }}>{TIMELINE[cur].desc}</p>
          </div>
        </div>
...
// ── Values ────────────────────────────────────────────────────────────────────
const VALUES = [
  { icon: '🛡️', title: 'Reliability',    desc: '99.9% uptime. Redundant infrastructure. 24/7 operations team. When your driver needs support at 2am, we are there.' },
  { icon: '💡', title: 'Innovation',     desc: 'Proprietary AI models trained on 1 billion+ km of real road data. We invest in R&D to keep our customers ahead.' },
  { icon: '👥', title: 'Customer First', desc: 'Every feature we build starts with a real customer problem. Open product roadmap. Monthly on-site visits by our product team.' },
  { icon: '🤝', title: 'Integrity',      desc: 'We never sell customer data. Transparent pricing. No hidden charges. When we make a mistake, we own it and fix it.' },
];
...
const ValCard = ({ icon, title, desc }) => {
  ...
  return (
    <div style={{ background: hover ? NAVY : '#fff', padding: '36px 28px', transform: hover ? 'translateY(-4px)' : 'none', transition: 'all 0.25s', cursor: 'default' }}
      ...
      <div style={{ width: '48px', height: '48px', background: hover ? 'rgba(200,16,46,0.18)' : 'rgba(200,16,46,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px', fontSize: '22px', transition: 'background 0.25s' }}>
        {icon}
      </div>
      ...
      <div style={{ fontSize: '13px', color: hover ? 'rgba(255,255,255,0.5)' : '#6b7280', lineHeight: 1.7, transition: 'color 0.25s', fontFamily: P }}>{desc}</div>
    </div>
  );
};
  ...
  return (
    <div>
      ...
      <main>
        ...
        {/* ── Existing homepage sections (pehle wale) ── */}
        <Hero />
        <Industries />
        <CaseStudies />

        {/* ── About content (end mein) ── */}
        <Ticker />
        <StatsSection />
        <CEOSection />
        <TimelineSlider />
